...
class User extends Base {
    ...
    public function detail(){
        ...
        if(IS_POST){
            if(!in_array($user['level'], [2,10,11,12,13,14]) && $user['level'] != I('post.level')){
                $_POST['invitation_code_own'] = getyqcodes();
            }elseif(in_array($user['level'], [2,10,11,12,13,14])){
                if($user['invitation_code_own'] == '0' || empty($user['invitation_code_own']) || $user['invitation_code_own'] == 0){
                    $_POST['invitation_code_own'] = getyqcodes();
                }
            }
            //  会员信息编辑
    ...
    public function robot(){
        $users = M('users');
        if(IS_POST){
            $is_robot_count = $users->where(['is_robot'=>1])->count();
            $dataPost = I('post.');
            if(empty($dataPost['nickname'])){
                $data['nickname'] = '机器人编号'.($is_robot_count+1);
            }
            if(empty($dataPost['password'])){
                $dataPost['password'] = '123456';
            }
            $data['password'] = encrypt($dataPost['password']);
            $data['level'] = $dataPost['superior_level'];

            if(!empty($dataPost['superior'])){
                $data['first_leader'] = $dataPost['superior'];
                $superior = $users->where(['user_id'=>$dataPost['superior']])->field('invitation_code_own,underling_number')->find();
                $data['invitation_code_others'] = $superior['invitation_code_own'];
            }
            if($dataPost['superior_level'] > 1){
                $data['invitation_code_own'] = getyqcodes();
            }
            $data['reg_time'] = time();

            $length = strlen($is_robot_count+1);
            $start = 19;
            if($length == 9){
                $data['mobile'] = $start.($is_robot_count+1);
            }else{
                $middle = '0';
                for ($i=$length; $i < 8; $i++) {
                    $middle = $middle.'0';
                }
                $data['mobile'] = $start.$middle.($is_robot_count+1);
            }
            ...
            $middle_id_card = '0';
            for ($i=$length; $i < 16; $i++) {
                $middle_id_card = $middle_id_card.'0';
            }
            $data['id_card'] = '1'.$middle_id_card.($is_robot_count+1);

            $data['registration_type'] = 3;
            if(!empty($$data['invitation_code_others'])){
                $users->where(['user_id'=>$dataPost['superior']])->setInc('underling_number');
            }
            $data['is_robot'] = 1;
            $res = $users->add($data);
            ...
            if($res){
                ...
                $this->error('添加失败',U('User/index'));exit;
            }
        }else{
            //上级
            $where['invitation_code_own'] = ['neq',0];
            $where['is_lock'] = 0;
            $superior_user = $users->where($where)->order('level asc')->getField('user_id,mobile,level');
            //等级
            $user_level = M('user_level');
            $level = $user_level->where('level_id>0')->order('level_id asc')->getField('level_id,level_name,describe');
            $this->assign('level',$level);
            $this->assign('superior_user',$superior_user);
            return $this->fetch();
        }
    }
