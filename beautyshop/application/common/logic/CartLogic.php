...
class CartLogic extends Model
{
    ...
    public function buyNow()
    {
        ...
        $new_price = $this->getNewprice($this->user_id,$this->goods['shop_price']);
        $buyGoods = [
        ...
        if (empty($this->specGoodsPrice)) {
            ...
            $new_price_two = $this->getNewprice($this->user_id,$this->specGoodsPrice['price']);
            $buyGoods['member_goods_price'] = $new_price_two;
    ...
    private function addNormalCart()
    {
        ...
        if ($userCartGoods) {
            ...
            $new_price = $this->getNewprice($this->user_id,$price);
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum, 'goods_price' => $price, 'member_goods_price' => $new_price]);
        } else {
            //如果该商品没有存在购物车
            if ($this->goodsBuyNum > $store_count) {
            ...
            $cartAddData = array(
                ...
                'member_goods_price' => $new_price,  // 会员折扣价 默认为 购买价
                'goods_num' => $this->goodsBuyNum, // 购买数量
                'add_time' => time(), // 加入购物车时间
                'prom_type' => 0,   // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                'prom_id' => 0,   // 活动id
    ...
    public function getNewprice($user_id,$price){
        $level_id = getDeCode($_COOKIE['level_id'],'key');
        if(!empty($level_id)){
            $level =  M('user_level')->where(['level_id'=>$level_id])->find();
        }

        // $level =  M('user_level')->alias('l')->join('__USERS__ u', 'u.level = l.level_id', 'RIGHT')->where(['u.user_id'=>$user_id])->getField('l.level_id');
